...
features <- features[,2]  # features: character vector (not a factor since colClasses is specified)
...
found <- grep("mean|std", features)  # including mean(), std(), meanFreq()
## found <- grep("mean[(]|std",features)  # including mean() and std() only
...
write.table(cbind(1L:length(found), data.frame(features[found])), file="features_avg.txt", 
